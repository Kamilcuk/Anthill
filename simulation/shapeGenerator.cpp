...
std::set<Point> ShapeGenerator::GenerateLine(const Point& start, const Point& end, int thickness)
{
	if(thickness < 0) throw std::runtime_error("Line thickness can't be negative");
	
	const float horizontal_change = (end.posX() - start.posX()),
		vertical_change = (end.posY() - start.posY()),
		len = sqrt(horizontal_change*horizontal_change + vertical_change*vertical_change),
	...
	while (x >= boundary_left && x <= boundary_right &&
		y >= boundary_bottom && y <= boundary_top)
	{
		std::set<Point> points_in_circle = GenerateCircle(Point(std::round(x), std::round(y)), 
			(unsigned)((float)thickness / 2.0));
